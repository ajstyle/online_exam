  ...
  todoApp.controller("BatchController", function($scope, $ionicPlatform, $stateParams, $cordovaSQLite, $location, md5, Auth) {
    ...
    $ionicPlatform.ready(function() {
      ...
      $cordovaSQLite.execute(db, query, [v]).then(function(result) {
        if (result.rows.length > 0) {
          for (var i = 0; i < result.rows.length; i++) {
      ...
      .success(function(data, status, headers, config) {
        ...
        $http.get('http://exam.imbueaura.com/index.php/api/quiz/bid/' + batchid + '/format/json')
          .success(function(data, status, headers, config) {
                            ...
                            if (data.status == "FALSE") {
                                ...
                                angular.forEach(ques, function (value, key) {
                                        ...
                                        $http.get('http://exam.imbueaura.com/index.php/api/printresult/rid/' + rid + '/format/json')
                                            .success(function (data, status, headers, config) {
                                                ...
                                                angular.forEach(result_q, function (values, keys) {
                                                    ...
                                                    $cordovaSQLite.execute(db, query, [rid, uid, quid, qids, category_name, qids_range, oids, start_time, end_time, last_response, time_spent, time_spent_ind, score, percentage, q_result, status, institute_id, photo, essay_ques, score_ind]).then(function (res) { }, function (err) {
                                                        console.error(err);
                                                    });
                                                });
                                            });
                                    ...
                                    angular.forEach(value.options, function (val, ke) {
                                        ...
                                        $cordovaSQLite.execute(db, query, [qid, val.oid, val.option, val.option_score, qtid]).then(function (res) { }, function (err) {
                                            console.error(err);
                                        });
                                    });
                                });
            ...
            $cordovaSQLite.execute(db, query, [batchid]).then(function (result) {
                console.log(result);
                if (result.rows.length > 0) {
  ...
  todoApp.controller("AccessController", function($scope, $ionicPlatform, $ionicLoading, $sce, $stateParams, $cordovaSQLite, $location, OptionId, Auth, $http, $ionicPopup) {
    ...
    $scope.nextItem = function($index, $qid) {
      ...
      var query_i = "SELECT DISTINCT rid, qid, question FROM Questions WHERE qid = ? and quid = ? ";
      $cordovaSQLite.execute(db, query_i, [$qid, $stateParams.quizId]).then(function (result) {
    ...
    $scope.showConfirm = function() {
      var confirmPopup = $ionicPopup.confirm({
        title: 'Confirm',
        template: 'Are you sure you want to Submit Exam?'
      });
   ...
   *  cordovaSQLite
   */
  todoApp.controller("DetailController", function($scope, $sce, $ionicPlatform, $stateParams, $cordovaSQLite, $location, md5, Auth, ionicMaterialMotion, $ionicLoading) {
    // Container for the categories in this Scope.
    $ionicLoading.show({
      template: '<ion-spinner icon="ripple" class="spinner-calm"></ion-spinner><p class = "loader1">Please Wait ...</p>'
      // noBackdrop: truecrescent
